...
def wafer_ID():
    ...
    full_path3 = '\\cvpmfsip01\S3E_Data\REL_EFR\SATA_CNE\CNE6_PC1\Bonafire_UID_EFR_Algo_V3_F0_73224'
        for path4 in os.listdir(full_path3):
            if(path4[0:16] == 'LUA_BURN_EXECUTE'):
                full_path4 = os.path.join(
                    full_path3, path4).replace('\\', '//')
                for path5 in os.listdir(full_path4):
                    if (path5[0:3] == 'DUT'):
                        full_path5 = os.path.join(
                            full_path4, path5).replace('\\', '//')
                        for path6 in os.listdir(full_path5):
                            ...
                            if path6[:10] == 'S3E_SysErr':
                                ...
                                line = line + 1
                                strr[line] = os.path.join(
                                    full_path5, path6).replace('\\', '//')
                                print strr[line]

    result_file = 'c:/' + str(time.localtime().tm_year) + '-' + str(
        time.localtime().tm_mon) + '-' + str(time.localtime().tm_mday) + '_Lua_Execuate.txt'
